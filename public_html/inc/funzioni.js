...
function okdupl(campo,i) {
//verifica se l'operatore vuole effettuare una DUPLICAZIONE  e non ha spuntato il record 
	if (campo=="Duplicazione") {
		errore=true;
		for (j=1; j <= i; j++) {
			selez="document.box.selez" + j + ".checked";
			if ( eval(selez) == true) {
		...
		if (errore==true) {
			alert("Selezionare i record da Duplicare");
			return false;
		}
		else {
			return true;
